...
**8 — Export as zip, with the migration's inverse as a switch.** Planned 2026-09-10, after the multi-select and the duplicates filter had made the archive something the reader reorganises rather than only reads.

One row in Settings, one dialog, one switch. The verbatim case is a snapshot — every `.md` and every attachment, byte for byte, in one file the user names through `CreateDocument` — and the switch is the inverse of `Migration`: **write each note's tags and title back into its text**, as `#hashtags` on a line of their own and a `# Heading` opening the note, for an editor that reads them there.

**A zip, not a folder.** A folder export was planned first and the zip beats it on every count. One file and one picker instead of a tree grant and a dated subfolder; a failure halfway leaves one file to delete, not a half-written folder. A zip entry carries an mtime where a SAF copy cannot, so each note's entry is stamped with its `updated` and the archive's dates travel with the files even to a tool that reads mtimes. `ZipOutputStream` over one stream is a few hundred milliseconds where 168 SAF creates were several seconds. Entry names are UTF-8 with the EFS flag and NFC as the filenames already are, under the folder's own name as a top-level directory — `Lyrics/Atlantik.md`, `Lyrics/attachments/…` — so unzipping does not spray 168 files into wherever it was opened.

**Export, not Backup, and the switch is off by default.** "Backup" promises a restore and the app has none; restoring is unzipping into the folder, which the user does, and the dialog says so in one sentence. And a backup is bytes: with the switch on the bodies are rewritten, and that copy is the archive prepared for another editor, not a copy of the archive. Both are useful, the default has to be the verbatim one, and the switch's label makes the transformation sound like what it is.

**The frontmatter is untouched either way.** The switch adds to the body; it removes nothing and rewrites no key. The other editor's own export had no frontmatter at all, and reproducing that shape would drop ten years of `created` for the sake of a format nothing needs. What the switch produces is the state the app held until 2026-09-09: both representations present. The shape per note is the one the original export wrote at the head of 65 notes — after the frontmatter's blank line, `# Title`, a blank line, the tag line, a blank line, the words. Heading only if the note has a `title:`; tag line only if it has tags; nothing derived from the file name, since nothing in the app derives a title from a name and the export must not start.

**No doubling.** 165 notes still carry their old hashtag line and would come out saying `#lyrics/snippet` twice. `Migration.carried` answers what a body already holds — which tags sit on lines of their own, and whether the first line that is not a tag line is a `# Heading` — and the export adds only the difference: a note whose body carries all its tags gets no tag line, a note that opens with any heading gets no heading. The hashtag grammar keeps its one owner: the export asks `Migration`, it does not run the regex, which is the way to not be the second caller the migration's comment warns about. A tag line that has to be added to a note that already opens with a heading goes under the heading, not above it. A body that carries some of its tags but not all — `Lyrics_Check`, after a rename — gains a line with the missing ones and keeps the line it had; the old line is text, as everywhere else.

**Nothing is stamped.** `updated` in the copy is whatever the file says: this is filing. A note the parser cannot reproduce byte for byte is copied verbatim even with the switch on, and counted separately in the result, since transforming a note the app has misread is how a copy stops being a copy.

**The property that makes it testable.** `Export.render(note, inline = false)` is `note.render()` byte for byte over the whole corpus. And for every note in `Lyrics_Inline`: migrate it, export it inline, migrate it again, and the frontmatter is the frontmatter — the tag half is an exact inverse, note for note, which is what lets the migration's dialog promise an export instead of an undo. On the other editor's side the body comes back identical for the 54 notes that had one tag line at the head; the 99 that had it at the foot get it back at the head, and a note that had two lines gets one, which is what an inverse cannot know and no tag lost. The title half is one-way by the migration's own rule — it leaves a titled note's heading alone — so on the archive the test is that no tag line is doubled and the heading appears once.

**Pieces.** `markdown/Export.kt`, pure, with the two tests; `Migration.carried`; `Vault.exportZip(destination, inline)`, which reads every note from disk rather than from the index so the copy is the file and not the parse, and finds `attachments/` without making one; a Settings row opening a dialog with the switch, the folder's note count, and the sentence about restoring — the attachment count is reported afterwards, since only the zip pass counts the files; the `CreateDocument` launcher with a suggested name of the folder and the date.
