...
**Decided on 2026-09-07: frontmatter only, as `CLAUDE.md` says.** The drawer shows `100%` on the 11 notes that declare it rather than on the 30-odd that mention it, and the app never writes a reconciliation. This deliberately under-counts, in exchange for the drawer never disagreeing with the files and for no later version being tempted to write frontmatter into 21 notes nobody edited.

One visible consequence to keep in view: a body-only `#100%` sits on a line the app hides as a tag line and is not chipped either, so it vanishes from the screen while staying on disk. If that grates once the chip row exists in phase 3, the fix is to chip percent tags without indexing them — never to start writing files.
