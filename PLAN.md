...
**The set the user edits is seeded from the frontmatter, never from the union.** `Note.tags` unions frontmatter and inline for the *index*; handing that union to `Frontmatter.withTags` would promote a body-only tag into the YAML the moment the user changed some unrelated chip — writing to lines nobody touched. For ordinary tags this never fires, the two agreeing everywhere. For the 21 body-only percent tags it fires immediately, which is how the hole was found.
...
**Percent tags found in a body are chipped and marked not-removable.** This is the fix the open question below named. They are displayed because the author can plainly see them in their own text; they are not indexed, so the drawer keeps agreeing with the frontmatter; and they are not members of the editable set, so no save can either delete them or promote them. Removing one would mean editing prose, which the app does not do.
...
## The question phase 6 answered

**Do body-only percent tags belong in the index?** 21 of them exist — `#100%` written in a note whose frontmatter does not list `100%`, because the Standard Notes export could not read `%` as part of a tag name. Indexing from the frontmatter alone, as `CLAUDE.md` says, means the drawer shows `100%` on 11 notes when the author has written it on far more; indexing from the body too means the drawer disagrees with the frontmatter, and saving a note would then be tempted to reconcile them — writing to 21 files the user did not edit, which is exactly the unbidden tidying the prime directive forbids.

**Decided on 2026-09-07: frontmatter only, and the app never writes a reconciliation.** The drawer shows `100%` on the 11 notes that declare it rather than on the 30-odd that mention it. This deliberately under-counts, in exchange for the drawer never disagreeing with the files.

**Completed in phase 6 with the fix that decision named: chipped, not indexed.** A body-only percent tag would otherwise vanish from the screen entirely — hidden as part of a tag line, and not chipped either — while sitting in plain sight on disk. So the chip row shows it, marked not-removable, and nothing else changes: the drawer's counts still come from the frontmatter, and the tag is excluded from the set a save may write. Display was the missing half; indexing and writing stay where they were.
